...
public class StudentStreamLab {
	public static void main(String[] args) {
		...
		List<Student> highGpaStudents = students.stream().filter(student -> student.gpa > 3.0)
				.collect(Collectors.toList());
		System.out.println("Students with GPA > 3.0: " + highGpaStudents);

		// Mapping: Extract names of students with GPA > 3.0
		List<String> highGpaStudentNames = students.stream().filter(student -> student.gpa > 3.0)
		...
		double averageGpa = students.stream().mapToDouble(student -> student.gpa).average().orElse(0.0);
		System.out.println("Average GPA: " + averageGpa);

		// Collecting: Collect all "Junior" students into a list
		List<Student> juniorStudents = students.stream().filter(student -> student.collegeYear.equals("Junior"))
				.collect(Collectors.toList());
